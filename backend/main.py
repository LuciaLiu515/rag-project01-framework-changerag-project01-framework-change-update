...
# 解决Pydantic模型名称冲突警告
app.model_config = {"protected_namespaces": ()}
...
async def embed_document(data: dict = Body(...)):
    try:
        ...
        vector_db_provider = data.get("vector_db_provider", VectorDBProvider.MILVUS.value)
        ...
        return {
            ...
            "vector_db_provider": vector_db_provider  # 添加向量数据库提供商到响应中
        }
...
async def search(
    ...
    provider: Optional[str] = Body(default=VectorDBProvider.MILVUS.value, description="向量数据库提供商")
):
    """执行向量搜索"""
    try:
        # Log the incoming search request details
        logger.info(f"Search request - Query: {query}, Collection: {collection_id}, Top K: {top_k}, Threshold: {threshold}, Word Count Threshold: {word_count_threshold}, Provider: {provider}")
...
async def parse_file(
    ...
    # New parse pipeline options
    parse_backend: str = Form(None),
    save_json: bool = Form(False),
    # Unstructured extras
    include_header_footer: bool = Form(None),
    infer_table_structure: bool = Form(None),
    extract_images_in_pdf: bool = Form(None),
    ...
    try:
        ...
        # If parse_backend specified, run the new Parse File pipeline.
        # Default stays the old ParsingService.
        if parse_backend:
            if parse_backend not in ["unstructured"]:
                raise HTTPException(status_code=400, detail=f"Unsupported parse_backend: {parse_backend}")

            parser = DocumentParseService()
            parsed_json = parser.parse_file(
                temp_path,
                backend=parse_backend,
                method=parsing_option,
                filename=file.filename,
                strategy=strategy or "hi_res",
        ...
        saved_path = None
        if save_json and parse_backend:
            os.makedirs("00-parsed-docs", exist_ok=True)
            ts = datetime.now().strftime('%Y%m%d%H%M%S')
            base = os.path.splitext(file.filename)[0]
            saved_path = os.path.join("00-parsed-docs", f"{base}_{parse_backend}_{ts}.json")
...
async def load_file(
    ...
    languages: str = Form(None),
    # LlamaParser extras
    llamaparser_model: str = Form(None),
    llamaparser_api_key: str = Form(None),
):
    try:
        # 保存上传的文件
        temp_path = os.path.join("temp", file.filename)
        ...
        languages_list = None
        if languages:
            # Accept comma-separated languages input, e.g. "eng,chi_sim"
            languages_list = [x.strip() for x in languages.split(",") if x.strip()]
        ...
        raw_text = loading_service.load_pdf(
            ...
            extract_images_in_pdf=extract_images_in_pdf,
            languages=languages_list,
            llamaparser_model=llamaparser_model,
            llamaparser_api_key=llamaparser_api_key,
        )
...
class GenerateRequest(BaseModel):
    ...
    vector_db_provider: Optional[str] = Field(default=None, description="向量数据库提供商")
