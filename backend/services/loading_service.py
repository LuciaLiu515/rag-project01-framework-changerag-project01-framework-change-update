...
class LoadingService:
    ...
    def load_pdf(
        ...
        *,
        ...
        extract_images_in_pdf: Optional[bool] = None,
        languages: Optional[list[str]] = None,
        # LlamaParser extras (optional dependency)
        llamaparser_model: Optional[str] = None,
        llamaparser_api_key: Optional[str] = None,
        **kwargs: Any,
    ) -> str:
        """
        ...
        try:
            ...
            elif method == "llamaparser":
                return self._load_with_llamaparser(
                    file_path,
                    model=llamaparser_model,
    ...
    def _load_with_unstructured(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            params: dict[str, Any] = {**strategy_params.get(strategy, {"strategy": "fast"}), **chunking_params}

            # Optional extras (best-effort; ignored by older unstructured versions)
    ...
    def _load_with_llamaparser(self, file_path: str, model: str = None, api_key: str = None) -> str:
        """Load PDF via LlamaParser (optional dependency).

        Notes:
            - This is best-effort and only works if `llama-parse` is installed and API key is provided.
            - For minimal repo changes, we don't hard-require the dependency.
            - Output is flattened to plain text and page_map becomes a single page.
        """
        try:
            ...
            raise ValueError(
                "LlamaParser requested but dependency is missing. Install `llama-parse`."
            ) from e
        ...
        if not api_key:
            raise ValueError("LlamaParser requires API key. Set LLAMA_CLOUD_API_KEY or pass api_key.")

        parser = LlamaParse(api_key=api_key, result_type="text", parsing_instruction=None)
        if model:
            # Some versions accept `model`.
            try:
                parser.model = model
            except Exception:
                pass
        ...
        # Minimal page map: treat as one page
        self.total_pages = 1
        self.current_page_map = [{"text": text, "page": 1, "metadata": {"source": "llamaparser"}}]
        return text
