...
class GenerationService:
    ...
    def _generate_with_local(
        ...
        """
        try:
            # 本地LLM不需要API密钥，使用空字符串
            client = OpenAI(
                api_key="empty",  # 本地模型不需要实际的API密钥
                base_url="http://192.168.1.9:8000/v1"  # 本地模型的API地址
            )
            ...
            response = client.chat.completions.create(
                model=self.models["local"][model_name],
                messages=messages,
                temperature=0.7,
                max_tokens=512
            )
            ...
            logger.error(f"Error generating with local LLM: {str(e)}")
            raise
    ...
    def generate(
        ...
        参数:
            provider: 模型提供商，可选值为"huggingface"、"openai"、"deepseek"、"local"
            model_name: 模型名称
            query: 用户查询
        ...
        try:
            ...
            if provider == "huggingface":
                ...
                response = self._generate_with_local(model_name, query, context)
            else:
